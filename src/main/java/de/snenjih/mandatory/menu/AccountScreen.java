...
public class AccountScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("mandatory.accounts.title").getString(),
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("mandatory.accounts.add").getString(),
    ...
    private void renderEmptyState(DrawContext ctx, int a) {
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("mandatory.accounts.no_accounts").getString(),
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (isActive) {
                ctx.drawTextWithShadow(textRenderer,
                        Text.translatable("mandatory.accounts.active").getString(),
            ...
            if (!isActive) {
                ...
                String  btnLbl = isSwitching ? "..." : Text.translatable("mandatory.accounts.switch").getString();
                int     btnW   = 60, btnH = 18;
    ...
    private void renderConfirmDelete(DrawContext ctx, AccountEntry entry,
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("mandatory.accounts.delete_confirm").getString(),
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("mandatory.accounts.delete_yes").getString(),
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("mandatory.accounts.delete_no").getString(),
            ...
            if (!isActive) {
                ...
                if (mx >= btnX && mx <= btnX + btnW && my >= btnY && my <= btnY + btnH) {
                    if (MinecraftClient.getInstance().world != null) {
                        switchError = Text.translatable("mandatory.accounts.in_game_warning").getString();
    ...
    private String resolveError(String code) {
        ...
        if (code.startsWith("no_network"))   return Text.translatable("mandatory.add_account.error.no_network").getString();
        if (code.startsWith("session"))      return Text.translatable("mandatory.add_account.error.refresh_failed").getString();
        return "Error: " + code;
    }
